...
class PerbaikanController extends Controller
{
    public function index()
    {
        $perbaikans = Perbaikan::select(
            ...
            ->with('perangkat', function ($query) {
                $query->select('id', 'karyawan_id', 'unit_id');
                $query->with('karyawan:id,nama');
                $query->with('unit:id,nama');
            })
            ->with('detail_perbaikans', function ($query) {
                $query->select('perbaikan_id', 'sparepart_id');
                $query->with('sparepart:id,kategori,tipe,kapasitas,merek,model');
            })
            ->orderByDesc('tanggal')
            ->get();
        ...
        return view('user.perbaikan.index', compact('perbaikans', 'perangkats'));
    }
    ...
    //     return view('user.perbaikan.create', compact(
    //         'perangkats',
    //         'motherboards',
    //         'prosesors',
    //         'rams',
    ...
    public function show($id)
    {
        ...
        return view('user.perbaikan.show', compact(
            'perangkat',
            'motherboards',
            'prosesors',
            'rams',
    ...
    public function store(Request $request)
    {
        ...
        Storage::disk('local')->put('public/uploads/' . $namaparaf, base64_decode($paraf));
        ...
        return redirect('user/perbaikan');
    }
